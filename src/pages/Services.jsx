  ...
  return (
    <section className="max-w-5xl mx-auto px-6 pt-20 pb-24">
      <div className="text-center mb-20 fade-up delay-1">
        ...
        <h1 className="font-serif text-4xl sm:text-6xl text-ink mt-6 leading-tight">
      ...
      <div className="reveal mb-20 bg-white p-10 rounded-3xl shadow-apple">
        <h2 className="text-2xl font-semibold text-ink mb-4">How I Solve Problems</h2>
      ...
      <ul className="grid md:grid-cols-3 gap-6 mb-20" role="list">
        {services.map((service, i) => {
          ...
          return (
            <li
              key={service.title}
              className={`reveal delay-${(i % 3) + 1} bg-white p-7 rounded-2xl shadow-apple border border-gray-50 flex flex-col items-start text-left hover:shadow-lg transition-shadow`}
            >
              <div className="w-11 h-11 rounded-xl bg-ink/[0.06] flex items-center justify-center mb-4">
      ...
      <div className="reveal mb-20">
        <h2 className="text-2xl font-semibold text-ink mb-8 text-center">Why Work With Me?</h2>
        <div className="flex flex-wrap justify-center gap-3">
          {["Modern Practices", "Responsive Design", "SEO-Focused", "Clean Code", "Accessibility First", "Mobile-First"].map((item) => (
            <span key={item} className="px-4 py-2 bg-ink/[0.05] text-ink rounded-full text-sm font-medium">{item}</span>
          ))}
        </div>
      </div>

      <div className="reveal text-center bg-black text-white p-12 rounded-3xl">
        ...
        <Link to="/contact" className="bg-white text-black px-8 py-3 rounded-full font-medium hover:opacity-90 transition">
          Get In Touch
        </Link>
      </div>
    </section>
